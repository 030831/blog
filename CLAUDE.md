...
트리(`CategoryTree`)는 작은 알약 버튼 + CSS 연결선으로 그립니다. 카드 격자로 만들었다가
빈 공간만 커져서 되돌린 적이 있습니다. 항목은 작게, 계층은 선으로 표현할 것.
깊이가 깊어질수록 글자 크기를 줄여(d1/d2/d3) 위계를 드러냅니다.
